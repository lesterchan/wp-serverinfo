...
* NEW: WordPress 7.0
* FIXED: Escape server/PHP/MySQL/memcached output to prevent XSS
* FIXED: Guard $_SERVER access to avoid warnings when keys are unset
* CHANGED: Remove jQuery dependency and inline the vanilla JS (removed serverinfo-js.js / serverinfo-js.dev.js)
* CHANGED: Require the manage_options capability for the admin page (was add_users)
* CHANGED: Modernize the admin page slug to wp-serverinfo
* CHANGED: Prefer the Memcached extension, falling back to Memcache
* CHANGED: Add direct-access (ABSPATH) guard
* CHANGED: Rebuild PHP tab from ini_get_all() instead of scraping phpinfo() output (no longer exposes environment variables / request headers)
* CHANGED: Bring the codebase up to WordPress Coding Standards (phpcbf + docblocks)
