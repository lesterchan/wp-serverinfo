...
Nothing of its own — no settings, no version row. WordPress itself records per-user Dashboard state when somebody closes, hides or reorders the widget, and deleting the plugin from the Plugins screen cleans that up too.
...
* NEW: An `uninstall.php`, so deleting the plugin leaves nothing behind on a single site or across a network
...
**The plugin stores nothing of its own** — no settings, no version row. There is still no settings screen. Deleting the plugin now clears the per-user dashboard state WordPress recorded whenever somebody closed, hid or reordered the widget, which 2.0.0 left in the database forever.
