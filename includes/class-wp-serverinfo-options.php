 ...
 *
 * WP-ServerInfo has no settings screen and nothing a site owner can change.
 * Everything it shows is read live from the host on each render -- that is the
 * whole plugin -- so there is no state to keep and no default to override. It
 ...
 * 2.1).
 *
 * The nearest thing to a setting is which memcached or Redis server the two
 * cache panels report on, and that stays a filter rather than becoming a form
 * field. A field would let anyone holding manage_options make the web server
 * open a connection to an arbitrary host and port and read the reply back on
 * the page, which is a request-forgery surface the plugin has no reason to
 * grow. Sites whose cache is not on localhost are running code already.
 *
 * wp_serverinfo_version is the pair of upgrade markers, and earns its place on
 ...
 */
class WP_ServerInfo_Options {

	/**
	 * Upgrade markers row, holding 'plugin' and 'db'. Autoloaded.
	 */
	const VERSION = 'wp_serverinfo_version';
	...
	public static function maybe_upgrade() {
		...
		if ( WP_SERVERINFO_VERSION === $plugin && WP_SERVERINFO_DB_VERSION === $db ) {
			return;
		}
